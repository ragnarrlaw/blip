...
class ExtractedMasterQuestion(BaseModel):
    canonical_id: str = Field(description="The question identifier, e.g., 'Q1' or 'Q3_a'.")
    question_type: str = Field(pattern="^(independent|scenario)$", description="Must be 'independent' or 'scenario'.")
    question_text: str = Field(description="The exact wording of the question.")
    scenario_text: Optional[str] = Field(None,
    ...
    model_answer: str = Field(description="A comprehensive, ideal answer to the question.")
    rubric: str = Field(
...
class ParsedAnswer(BaseModel):
    canonical_id: str = Field(description="The canonical ID of the master question (e.g., 'Q6_1').")
    student_answer: str = Field(description="The isolated text the student wrote for this specific question.")
    is_blank: bool = Field(description="True if the student completely skipped or left this sub-question blank.")

class ParsedStudentSubmission(BaseModel):
    ephemeral_id: str = Field(description="The temporary ID assigned to this raw submission (e.g., 'RAW_01').")
...
class GradingFlag(BaseModel):
    type: str = Field(description="Flag type: 'low_confidence', 'alternative_solution', 'unclear_response', or 'grading_discrepancy'.")
    detail: str = Field(description="Explanation of the flag.")

class GradingResult(BaseModel):
    mark: float = Field(description="Numeric mark awarded.")
    criterion_scores: List[CriterionScore] = Field(description="Itemized criteria breakdown.")
    ...
    flags: List[GradingFlag] = Field(default_factory=list, description="Any evaluation flags.")

class BatchStudentEvaluation(GradingResult):
    item_tag: str = Field(description="The ephemeral tag matching the student answer (e.g., 'ANS_01').")
